...
require_once MW_PATH . '/include/version.php';

function trackback($url, &$post)
{
    global $xoopsConfig;
    if ($post->isNew() || 0 == $post->getApproved() || 1 != $post->getStatus()) {
        return;
    }
    $util = new RMUtils();
    //$url = urlencode($url);
    $title = urlencode($post->getTitle());
    $excerpt = urlencode($util->filterTags('' != $post->getExcerpt() ? htmlentities($post->getExcerpt()) : '[...] ' . htmlentities(mb_substr($util->filterTags($post->getText()), 0, $mc['tracklen'])) . ' [...]'));
    ...
    $http_request .= 'Content-Length: ' . mb_strlen($querys) . "\r\n";
