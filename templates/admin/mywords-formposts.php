<h1 class="cu-section-title"><?php $edit ? _e('Edit Post', 'mywords') : _e('Create Post', 'mywords'); ?></h1>
...
<div class="mw_permacont <?php if (!$edit): ?>mw_permainfo<?php endif; ?>" id="mw-perma-link">
    <?php if ($edit): ?>
        ...
        <a href="#" onclick="window.open('<?php echo $post->permalink(); ?>', 'preview');" class="btn btn-default btn-mini">
            <?php if ('publish' === $post->getVar('status')) {
    _e('View Post', 'mywords');
} else {
    _e('Preview Post', 'mywords');
} ?>
        </a>
    <?php else: ?>
...
<br>
    ...
    <div class="cu-box" id="format-video"<?php echo $edit && 'video' !== $post->format ? ' style="display: none;"' : ''; ?>>
...
<div class="cu-box box-default">
    ...
    <div class="mw_bcontent box-content">
        <table width="100%" cellpadding="0" cellspacing="0">
            <tr class="outer">
                <td width="50%">
                    <label><input type="checkbox" name="comstatus" value="1" checked> <?php _e('Enable comments for this post', 'mywords'); ?></label>
                </td>
                <td>
                    <label><input type="checkbox" name="pingstatus" value="1" checked> <?php _e('Allow trackbacks for this post', 'mywords'); ?></label>
                </td>
            </tr>
        </table>
    </div>
