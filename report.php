...
if ($mwSettings->reports <= 0) {
    $common->uris()->redirect_with_message(__('Sorry, you are not allowed to perform this action.', 'mywords'), MW_URL, RMMSG_WARN);
}
...
if ($id <= 0) {
    $common->uris()->redirect_with_message(__('You must provide a valid post ID', 'mywords'), MW_URL, RMMSG_WARN);
}

$post = new MWPost($id);
if ($post->isNew()) {
    $common->uris()->redirect_with_message(__('Specified post does not exists!', 'mywords'), MW_URL, RMMSG_ERROR);
}
...
if ('submit' == $action) {
    ...
    if ($user <= 0) {
        if ('' == trim($name) || '' == trim($email)) {
            $common->uris()->redirect_with_message(__('You must provide your name and your email!', 'mywords'), $post->permalink(), RMMSG_WARN);
        }

        if (false === checkEmail($mail)) {
            $common->uris()->redirect_with_message(__('You must provide a valid email!', 'mywords'), $post->permalink(), RMMSG_WARN);
        }
    }

    if ('' == trim($title) || '' == trim($content)) {
        $common->uris()->redirect_with_message(__('The main reason and details for report are required', 'mywords'), $post->permalink(), RMMSG_WARN);
    }

    // Check captcha if exists
    if ($common->services()->service('captcha')) {
        if (!$common->services()->captcha->verify()) {
            $common->uris()->redirect_with_message(__('CAPTCHA challenge failed! Please try again', 'rmcommon'), $post->permalink(), RMMSG_ERROR);
        }
    }
    ...
    if ($total > 0) {
        $common->uris()->redirect_with_message(__('Another similar report from you already exists', 'rmcommon'), $post->permalink(), RMMSG_ERROR);
    }
    ...
    if ($report->save()) {
        $common->uris()->redirect_with_message(__('Your report has been sent. Thank you!', 'rmcommon'), $post->permalink(), RMMSG_SUCCESS);
    } else {
        $common->uris()->redirect_with_message(__('Your report could not been sent. Please try again.', 'rmcommon'), $post->permalink(), RMMSG_ERROR);
    }
